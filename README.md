...
当前五组特征码（共七个写入位置）已在以下 QQ 版本完成只读扫描验证：
...
| 扫描结果 | 程序行为 |
| --- | --- |
| 五组原始特征码均达到各自要求的匹配数量 | 允许安装 |
| 五组补丁特征码均达到各自要求的匹配数量 | 识别为已安装 |
...
- 安装前要求每个目标中的五组原始特征码分别达到声明的精确匹配数量。
- 普通撤回只移除明确携带服务器通知标记的五个调用点，本人操作使用的本地调用保持原样。
...
QQ `9.9.33-51728` 中 `ReplaceToRecallMsg` 的 `by_notify` 调用参数重新定位；本人操作语义同时
对照了 [xiyuesaves/lite-tools](https://github.com/xiyuesaves/lite-tools) 对
`isSelfOperate` 的处理。
